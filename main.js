...
(async() => {
    ...
    while (true) {
        ...
        await mouse_click(right_clicks, 'right')
        await mouse_click(left_clicks, 'left')
...
async function mouse_click(clicks, button) {
    /* The above code is clicking on the tiles in the left_clicks array. */
    for (let i = 0; i < clicks.length; i++) {
        try {
            let tileNb = left_clicks[i][0] * boardSize[0] + left_clicks[i][1]
            await page.click('#tile' + tileNb, { button: button })
        } catch (error) {
            console.log("Error on mouse click");
        }
    }
